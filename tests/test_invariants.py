...
#   surgical: the file this route writes — C-13 sweeps it and proves the write
...
def test_every_file_write_declares_a_formatting_stance() -> None:
    """Canary: each file writer either gets the C-13 sweep or says why not.
    ...
    assert not undeclared, (
        ...
        "entry needs 'surgical' (the file it writes; C-13 sweeps it); a route that rewrites a file by nature "
        "needs a 'whole_file_reason'"
    )
    assert SURGICAL, "no write route claims a surgical write — did the probe table lose its C-13 coverage?"
...
async def test_single_entry_write_leaves_every_other_byte_alone(
    ...
    """C-13: one entry changed means one contiguous region of the file changed.
